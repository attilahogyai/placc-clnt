export default function() {
  ...
  */
  ...
  this.namespace = '/api';
  this.get('/japi/level/:id', (schema,request) => {
    return schema.level.find(request.param.id);
  });
  this.get('/japi/building/:id', (schema,request) => {
    return schema.building.find(request.param.id);
  });
  this.get('/japi/company', (schema, request) => {
    return schema.company.all();
  });

  this.get('/japi/company/:id', (schema, request) => {
